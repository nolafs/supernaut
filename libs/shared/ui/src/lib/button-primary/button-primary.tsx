...
export function ButtonPrimary({label, size, isDisabled = true, hasIcon = false, onClick}: ButtonPrimaryProps) {
  return (
    <button className={cn('font-medium flex item-center space-x-2 group',
      ...
      <div className={'flex flex-col'}>
        <span>{label}</span>
        <div className={cn("max-w-xs h-[2px] md:mt-1", (isDisabled) ? 'bg-secondary' : styles.decorLine, (size === 'lg') && 'lg:mt-2')}></div>
      </div>
      {hasIcon && (
        <div className={cn('w-8 h-8 md:w-9 md:h-9 group-hover:ml-3 ease-in-out duration-300', (size === 'lg') && 'lg:w-12 lg:h-12')}>
