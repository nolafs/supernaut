...
export function ButtonPrimary({label, size, isDisabled = true, hasIcon = false, onClick}: ButtonPrimaryProps) {
  return (
    <button className={cn('font-medium flex item-center space-x-2',
      (isDisabled) ? 'text-secondary cursor-not-allowed' : 'text-primary',
      (size === 'sm') && 'text-normal md:text-lg',
      (size === 'md') && 'text-xl md:text-2xl' ,
      (size === 'lg') && 'text-3xl md:text-4xl lg:text-5xl' , styles.container)} disabled={isDisabled}>
      <div className={'flex flex-col'}>
        <span>{label}</span>
        <div className={cn("max-w-xs h-[2px]", (isDisabled) ? 'bg-secondary' : 'bg-white', (size === 'lg') && 'lg:mt-2')}></div>
      </div>
      {hasIcon && (
