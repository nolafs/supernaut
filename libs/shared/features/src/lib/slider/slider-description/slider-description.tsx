...
export function SliderDescription({id, title, description}: SliderDescriptionProps) {
  ...
  const split = useRef<SplitText | any>();
  console.log('id', id)
  ...
  useGSAP(() => {
    ...
    gsap.from([split.current.chars, '.body' ], {opacity: 0, y:"100%", duration:0.5, stagger: 0.05, ease: 'power3.inOut'});
  ...
  return (
    <div ref={main} className={'absolute text-primary bottom-0 mx-6 md:mx-12 mb-6 md:mb-14 flex flex-col md:space-y-7 z-20'}>
      <h2 className={'header text-3xl md:text-[48px] font-medium tracking-tighter overflow-hidden p-1'}>{title}</h2>
